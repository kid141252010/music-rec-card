...
为了规避商业字体带来的版权风险，本项目默认集成了开源字体 **思源黑体 (Source Han Sans SC)**。
如果你想使用自定义字体（例如苹果的 `PingFang.ttc` 等其他自己喜欢的字体），有以下两种方式：

1. **放到 font 文件夹（推荐）**：直接将你需要用到的 `.ttf`、`.ttc` 或 `.otf` 字体文件放入与脚本同级的 `font/` 文件夹中。程序运行时会自动探测并优先加载该文件夹内的单文件字体。
2. **命令行指定**：在生成卡片时通过 `--font` 参数指定字体路径。
   ```bash
   python music_card_gen.py --platform ncm --mode daily --font "C:\Path\To\MyCustomFont.ttf"
   ```
   *注意：使用此命令后，程序会自动将该字体复制到 `font/` 目录下，并清空旧的自定义字体。下次直接运行无需再次指定。*
